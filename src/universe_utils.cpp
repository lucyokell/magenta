...
Rcpp::List population_get_genetics_ibd_df_n(Rcpp::List paramList) {
  ...
  for(unsigned int i = 0; i < (pop_size-1); i++) {
    ...
    for(unsigned int j = i+1; j < pop_size; j++) {
      if(bitsets[j].size()) {
      pibd_i = pibd_d_i = 0;
      for(unsigned int k = 0; k < bitsets[i].size(); k++){
        pibd_i = pibd_i + Strain::ibd_distance_of_bitset_a_and_x(bitsets[i][k], bitsets[j].begin(), bitsets[j].end());
      }
      for(unsigned int k = 0; k < bitsets_d[i].size(); k++){
        pibd_d_i = pibd_d_i + Strain::ibd_distance_of_bitset_a_and_x(bitsets_d[i][k], bitsets_d[j].begin(), bitsets_d[j].end());
      }
  ...
  for(unsigned int c = 0; c < pop_size; c++) {
    pibd[c] /= (contacts[c] * u_ptr->parameters.g_barcode_length);
    pibd_d[c] /= (contacts_d[c] * u_ptr->parameters.g_barcode_length);
  }
...
Rcpp::List population_get_genetics_df_n(Rcpp::List paramList) {
  ...
  std::vector<unsigned int> last_treatment(pop_size, 0);
  std::vector<unsigned int> states(pop_size, 0);
  std::vector<std::vector<unsigned int> > barcodes(pop_size);
  ...
  std::vector<Strain> strains_i;
  std::vector<boost::dynamic_bitset<> > bitsets_i;
  std::vector<unsigned int> barcodes_i;
  std::vector<unsigned int> barcodes_not_u_i;
  
  for (unsigned int el = 0; el < pop_size; el++) {
    
    // who are we sampling
    i = static_cast<unsigned int>(pop_sample[el]);
    ...
    barcodes_not_u_i.reserve(strains_i.size());
    ...
    // and then the barcodes and then assign them  into the barcodes vector
    for(auto b : bitsets_i) {
      boost::to_block_range(b, std::back_inserter(barcodes_i)); 
    }
    ...
    for(unsigned int bu = 0; bu < strains_i.size(); bu++) {
      if(strains_i[bu].get_m_strain_infection_status() != Strain::SUBPATENT) {
        boost::to_block_range(bitsets_i[bu], std::back_inserter(barcodes_not_u_i)); 
      }
    }
    
    // and then the unique size
    q_i = q_fun(u_ptr->parameters, ages[i], u_ptr->population[i].get_m_ID());
    std::sort(barcodes_not_u_i.begin(), barcodes_not_u_i.end());
    coi_detected_micro[el] = round(q_i * (std::unique(barcodes_not_u_i.begin(), barcodes_not_u_i.end()) - barcodes_not_u_i.begin()));

    // similar for all
    std::sort(barcodes_i.begin(), barcodes_i.end());
    coi[el] = std::unique(barcodes_i.begin(), barcodes_i.end()) - barcodes_i.begin();

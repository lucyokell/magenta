...
eqInits <- lapply(bm$EIRY_eq,function(x){return(Equilibrium_Init_Create(age.vector = c(0,0.25,0.5,0.75,1,1.5,1.75,2,3.5,5,7.5,10,15,20,30,40,50,60,70,80,90,100),
                                                                             het.brackets = 5,ft = 0.4,EIR = x,model.param.list = mpl))})

## Next create the list of near equilibrium steady state
eqSSs <- lapply(eqInits,function(x){return(Equilibrium_SS_Create(eqInit = x, end.year = 10))})
